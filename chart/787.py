...
class Solution:
    def findCheapestPrice(self, n: int, flights: List[List[int]], src: int, dst: int, k: int) -> float:
        book = [[float('inf')] * n for _ in range(n)]
        for flight in flights:
            book[flight[0]][flight[1]] = flight[2]
        for i in range(0, n):
            book[i][i] = 0
        ...
        for limit in range(k + 1):
            ...
            for i in range(n):
                for j in range(n):
                    dist[j] = min(dist[j], clone[i] + book[i][j])
        return dist[dst] if dist[dst] != float('inf') else -1


s = Solution()
print(s.findCheapestPrice(n=17, flights=[[0, 12, 28], [5, 6, 39], [8, 6, 59], [13, 15, 7], [13, 12, 38], [10, 12, 35],
