...
class Solution:
    def pacificAtlantic(self, heights: List[List[int]]) -> List[List[int]]:
        ...
        m_book = [[0] * m for _ in range(n)]

        for i in range(n):
            for j in range(m):
                book = [[1] * m for _ in range(n)]
                l, r = 0, 0
                q = [[i, j]]
                while q:
                    node = q.pop(0)
                    if m_book[node[0]][node[1]] == 3:
                        ans.append([i, j])
                        break
                    book[node[0]][node[1]] = 0
                    for s in step:
                        x, y = node[0] + s[0], node[1] + s[1]
                        if 0 <= x < n and 0 <= y < m:
                            if book[x][y] and heights[x][y] <= heights[node[0]][node[1]]:
                                    q.append([x, y])
                            continue
                        if x < 0 or y < 0:
                            l = 1
                            m_book[i][j] = m_book[i][j] | 1
                        if x >= n or y >= m:
                            r = 1
                            m_book[i][j] = m_book[i][j] | (1 << 1)
                    if l and r:
                        ans.append([i, j])
                        m_book[i][j] = 3
                        break
...
s = Solution()
print(s.pacificAtlantic([[12, 7, 7, 14, 6, 17, 12, 17, 8, 18, 9, 5],
